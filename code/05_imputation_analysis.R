...
#re-organize the weather data by variable
tmin <- purrr::map(weather, 'Tmin') %>% 
  bind_cols()

tmax <- purrr::map(weather, 'Tmax') %>% 
  bind_cols()
...
#gsod
tmin_gsod <- purrr::map(weather_gsod, 'Tmin') %>% 
  bind_cols()

tmax_gsod <- purrr::map(weather_gsod, 'Tmax') %>% 
  bind_cols()
...
tmin <- cbind(tmin, tmin_gsod)
tmax <- cbind(tmax, tmax_gsod)
precip <- cbind(precip, precip_gsod)


#make sure tmin, tmax and precip have columns called Day, Month, Year, Date
tmin$Day <- weather[[1]]$Day
tmin$Month <- weather[[1]]$Month
tmin$Year <- weather[[1]]$Year
tmin$Date <- weather[[1]]$Date

tmax$Day <- weather[[1]]$Day
tmax$Month <- weather[[1]]$Month
tmax$Year <- weather[[1]]$Year
tmax$Date <- weather[[1]]$Date
...
weather_info <- select(weather_info, id, Name, Latitude, Longitude, Elevation)
gsod_info <- select(gsod_info, id, Name, Latitude, Longitude, Elevation)
#combine
...
period_list <- lapply(weather_info$id, function(x){
  ...
  longest_coverage <- na.contiguous(ts(data.frame(tmin = tmin[,x], tmax = tmax[,x], precip = precip[,x])))
...
test <- lapply(1:nrow(weather_info), function(x){
  sum(is.na(precip[period_list[[x]][1]:period_list[[x]][2], weather_info$id[x]]))+
    sum(is.na(tmin[period_list[[x]][1]:period_list[[x]][2], weather_info$id[x]]))+
    sum(is.na(tmax[period_list[[x]][1]:period_list[[x]][2], weather_info$id[x]]))
} )
sum(unlist(test))
...
weather_list <- list(list(tmin, additional_args_temp), 
                     list(tmax, additional_args_temp), 
                     list(precip, additional_args_rain))
...
eval_list <- lapply(weather_list, function(x){
  weatherImpute::get_eval_one_station(weather = x[[1]], 
